...
def DIN(dnn_feature_columns, history_feature_list, dnn_use_bn=False,
    ...
    :param dnn_feature_columns: An iterable containing all the features used by deep part of the model. 模型的输入特征列表
    :param history_feature_list: list,to indicate  sequence sparse field 历史行为数据的特征列表（用于计算用户兴趣历史）
    :param dnn_use_bn: bool. Whether use BatchNormalization before activation or not in deep net 是否在DNN中使用Batch Normalization
    :param dnn_hidden_units: list,list of positive integer or empty list, the layer number and units in each layer of deep net DNN网络中的隐藏层单元数
    :param dnn_activation: Activation function to use in deep net DNN网络的激活函数
    ###### 与注意力机制相关的配置  #########
    :param att_hidden_size: list,list of positive integer , the layer number and units in each layer of attention net
    :param att_activation: Activation function to use in attention net
    :param att_weight_normalization: bool.Whether normalize the attention score of local activation unit.
    #####################################
    :param l2_reg_dnn: float. L2 regularizer strength applied to DNN L2正则化项的系数，用于DNN
    :param l2_reg_embedding: float. L2 regularizer strength applied to embedding vector L2正则化项的系数，用于嵌入层
    :param dnn_dropout: float in [0,1), the probability we will drop out a given DNN coordinate. DNN中的dropout比率
    :param seed: integer ,to use as random seed. 随机种子
    :param task: str, ``"binary"`` for  binary logloss or  ``"regression"`` for regression loss 任务类型（binary：二分类任务，regression：回归任务）
